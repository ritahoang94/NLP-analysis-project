...
# WORD EMBEDDING------
# 
w2v.model <- h2o.word2vec(model_id = 'w2v_e20_v150_w25_f5',words, sent_sample_rate = 0, epochs = 20,vec_size = 150, window_size = 25, min_word_freq =5) #vec_size:min_word_freq:word_model:
...
aml2 <- h2o.automl(x = names(label.vecs), y = 'label',
                  training_frame = data,  max_models=5,nfolds=5, seed = 1234)
# save model
leader <- h2o.saveModel(object=aml2@leader, path=getwd(), force=TRUE)
...
a<-predict(test.data,w2v.model,aml2@leader)
...
write.table(final_df,"ensemble_20182010_w2v_e20_v150_w25_f5.txt",sep = " ", row.names = F,col.names = F, quote = F)
